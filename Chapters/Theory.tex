...
also referred to as the \vname{\qfun}. The approximate \qfun is used
...
\begin{description}
  \item[Search] The tree policy is invoked to traverse the policy tree
    ...
    selects an action based on the current \qfun approximation. Once an
  ...
  \item[Backpropagation] The value estimate obtained from the rollout
    simulation is passed back up the tree to update the \qfun estimate on
...
This procedure describes the outer loop of the algorithm. Starting at the initial belief, $b_0$,
the algorithm performs $n$ iterations of \ac{mcts} to approximate the \qfun.
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        \Procedure{Plan}{$b_0$}
            \For{$i \in 1:n$}
                \State $s \gets \text{sample from }b_0$
                \State $\Call{Simulate}{s, b_0, d_\text{max}}$
            \EndFor
            \State $\textbf{return } \underset{a}{\argmax}\, Q(b_0a)$
        \EndProcedure\vspace{10pt}
