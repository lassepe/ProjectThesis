...
a_1, o_1, a_2, o_2,\dots$).\todo{Should I also add the formal definition using
the determinized simulative mode? May be already implementation details (even
though distribution details matter)} In \ac{despot}, scenarios are used to
obtain a sparse presentation of the future. \Cref{fig:despot-tree} depicts
a partially constructed \ac{despot} $\mathcal{D}$ rooted at the current belief,
$b_0$, obtained under two scenarios (marked in blue and orange). Each node
implicitly represents a particle belief constructed from the distribution of
outcomes encountered under all scenarios passing through that node.

The tree is constructed by sampling an initial state under each scenario,
$\phi_i$. Starting from the corresponding initial, for each scenario \emph{all
actions} $a \in \aspace$ are simulated one step into the future. For the
observations encountered during these determinized simulations a new node is
created one level deeper and the corresponding state-outcomes are recorded
to form a particle belief at every node. As result, the tree considers all
...
to converge to the true belief tree. In practice, however, a limited number of
scenarios sufficiently approximates the true belief tree such that the optimal
decision computed on $\mathcal{D}$ is a near-optimal decision for the dense
problem.
...
\begin{algorithm}[htpb]
  ...
  \begin{algorithmic}[1]
    ...
    \Procedure{BuildDespot}{$b_0$}
      \State $\Phib_{b_0} \gets K \text{ scenarios sampled from } b_0$
