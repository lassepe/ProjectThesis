...
introduce some of the \todo{missing word?}throughout this work. It should be noted that in the
interest of conciseness we focus this theoretical introduction on only the main
tools used. Fundamental concepts of statistical inference and tools like
...
uncertainty. As is common in literature, we denote quantities at time \todo{double word?}time $t$
...
but instead at every time step $t$ receives an \vname{observation} $o_t$,\todo{comma here seems weird}
...
is too complex to be evaluated analytically, Monte Carlo integration\todo{maybe mention the name "particle filter" here?} may be
...
to a wide range of problems. This work makes use of two of the most elaborate\todo{see other note on "elaborate"}
...
\emph{curse of dimensionality}\todo{I don't think you defined "curse of dimensionality"} by sampling states from root belief and
...
that determinizes the outcome of a random process. Loosely speaking\todo{I don't think you need "loosly speaking" here}, under
...
a sparse presentation of the future. \Cref{fig:despot-tree} depicts a partially
...
\todo[inline]{@zach: Can you give me some feedback on how many mathematical
details I should include here? I would prefer to describe \ac{despot} on an
abstract level that provides intuition. The mathematically precise version is
already formulated in \cite{somani2013despot}. However, I have a hard time
discussing the pseudo-code in \cref{alg:despot} without introducing all the
math / symbols / concepts (to be able to discuss the difference between $\ell$
vs $L$ and $\mu$ vs $U$.)}
\todo[author=ZS]{I think you should not spend very much space on the specifics of the algorithm - I don't think I would even put pseudocode in. I would just say in prose what each node represents (using L,U, and V instead of l, mu, and v), and how it chooses what to expand.}

A pseudo code representation of the algorithm is provided in \Cref{alg:despot}.

\todo[inline]{Introduce variables}
...
\todo[inline]{Finish once it is clear how to handle the "notation" overhead.}

% The \text{BuildDespot} procedure incrementally constructs a \ac{despot}
% $\mathcal{D}$ rooted at $b$ by considering $K$ scenarios. Initially,
% $\mathcal{D}$ only contains a single belief node $\Phib_b$ obtained by sampling
% from $b_0$ under each scenario. The upper and lower bounds, $U$ and $L$, on the
...
implementational\todo{@zach is "implementational" even a word?}\todo[author=ZS]{Haha, no - there is probably not one word that can express this well. Maybe "expressiveness" or "convenience" get close, but I would just use a couple sentences for it.} and computational
