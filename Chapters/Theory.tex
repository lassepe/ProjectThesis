...
receives \vname{rewards} $r$ based on the state-transition and the action taken
...
$\sspace$, integrals are to be replaced by sums. In cases where this update
rule is too complex to be evaluated analytically, a particle filter may be used
for approximate inference \cite{kochenderfer2015decision,
...
representation of the future. \Cref{fig:despot-tree} depicts a partially
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
      \Procedure {ActionProgWiden}{$h$}
          \If{$|C(h)| \leq k_\text{a} N(h)^{\alpha_\text{a}}$}\label{alg:aprogwiden:lin:check}
