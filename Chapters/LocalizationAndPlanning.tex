...
\Cref{fig:lp-value-sem} shows the mean and \ac{sem} of the cumulative
...
\begin{figure}[H]
  \centering
  \includegraphics[scale=1.0]{roomba_plots/lp_value_sem_eval_plot.pdf}
  \caption{Mean and \acf{sem} for each policy introduced in \cref{sec:lp-solutions}.}
  \label{fig:lp-value-sem}
\end{figure}

While above the performance is compared by the disconte
...
\begin{itemize}
  ...
  \item baseline policies don't totally fail: there seems to be enough passive
        info gathering that these things work automatically
\end{itemize}
